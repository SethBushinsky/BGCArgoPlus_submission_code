...
def density_inversion(argo_n, processed_fig_dir, outlier_df, verbose):
    ...
    if bad_data>0:
        ...
        for p in unique_profiles:
            ...
            plt.savefig(f'{processed_fig_dir}/{str(wmo)}/{plot_filename}.png')
            plt.close()
            plt.clf()
    if verbose:
...
def bottom_oxygen_check(argo_n, group_outlier_dir, outlier_df, verbose):
    ...
    for p in deriv_df['p'].unique(): # range(137,141):#  
        ...
        if np.any(outlier_index):
            ...
            ax.set_title('Profile ' + str(p))
            ax = fig.add_subplot(1,4,2)

            ax.plot(sal_nonan, pres_nonan, 'k-', marker='.')
            ax.set_ylim([max_pres, min_depth])

            axo = fig.add_subplot(1,4,3)
            ...
            if np.any(outlier_index):
                ...
                axo.set_title('Red = Removed points')
...
def sensor_flag_wrapper(sprof_path, file, flags_to_remove, output_dir, verbose=False):
    ...
    try:
        ...
        try:
            ...
            print('Failed to add sensor info for ' + str(file) + ' check if meta.nc file exists')
    ...
    if verbose:
        print('Saving out filtered file')
    try:
