...
# based on https://github.com/ethan-campbell/Weddell_polynya_paper

# Check out:
# - https://github.com/pyoceans/python-oceans/blob/main/oceans/sw_extras/sw_extras.py for non-sea ice areas
...
def vert_prof_eval(var,z_coor,z_or_z_range,interp_method='linear',extrap='NaN',
    ...
    # If ref_depth (=z_or_z_range) is an integer or "shallowest":
    ...
    # If ref_depth (=z_or_z_range) is a range 
...
def calc_mld(   pres_p,
                sigma_t_p,
                ref_depth=10,
                ref_range_method='interp',
                ref_reject=False,
                sigma_theta_crit=0.03,crit_method='interp',
                crit_interp_initial_spacing=1.0,crit_interp_final_spacing=0.01,
                bottom_return='bottom',
                verbose_warn=False,verbose_error=False):
    """ Compute mixed layer depth (MLD) given a vertical profile of sigma-theta (potential density anomaly).
    
    Args:
        pres_p: Pressure profile (array) 
                note that a positive, monotonically increasing Pressure vector is required

        sigma_t_p: Potential density profile (array) 
    ...
    """
    sigma_theta = sigma_t_p
    z_var = pres_p
    
    # Warnings (is verbose_warn set to true)
    if verbose_warn:
        # Warning if there's NaN values in the input pressure or density data
        if any(isnan(sigma_theta.any()) or any(isnan(z_var.any()))):
            print('Warning: sigma-theta or depth vector contains NaNs.')

        # Warning if ref_depth is set to "shallowest" and shallowest depth value is >= 20 m
        if ref_depth == 'shallowest':
            if z_var[0] >= 20: print('Warning : shallowest measurement is 20 m or higher.')
 
        # Warning if ref_depth is set to a range of values and shallowest depth value is outside that range (deeper)
        elif not isinstance(ref_depth,tuple):
            if z_var[0] > ref_depth:
                if not ref_reject: print(   'Warning : '
                                            'reference depth is above shallowest measurement.')
                else:              return NaN

        # Warning if ref_depth is set to a string that is shallower then the shallowest measurement
        elif not ref_reject:
            if z_var[0] > ref_depth[1]:
                if not ref_reject: print(   'Warning : '
                                            'reference depth range is above shallowest measurement.')
                else:              return NaN

    # Determine MLD density:
    # ref_depth is set to shallowest profile value:
    if ref_depth == 'shallowest':
        rho_mld = sigma_theta_crit + sigma_theta[0]

    # ref_depth is set to a range of values:
    elif not isinstance(ref_depth,tuple):
        rho_mld = sigma_theta_crit + vert_prof_eval(sigma_theta,z_var,
                                                    ref_depth,
                                                    interp_method='linear',extrap='nearest',
                                                    verbose_warn=True,verbose_error=True)
    
    # ref_depth is set to an integer value:
    else:
        rho_mld = sigma_theta_crit + vert_prof_eval(sigma_theta,z_var,
                                                    ref_depth,
                                                    interp_method='linear',extrap='nearest',
                                                    avg_method=ref_range_method,avg_spacing=0.1,
                                                    verbose_warn=True,verbose_error=True)

    # Determine MLD based on MLD density:
    mld_found = depth_at_which( sigma_theta,z_var,rho_mld, 
                                method=crit_method,
                                top=0.0,bottom='bottom',
                                interp_initial_spacing=crit_interp_initial_spacing,
                                interp_final_spacing=crit_interp_final_spacing,
                                verbose_warn=True,verbose_error=True)
